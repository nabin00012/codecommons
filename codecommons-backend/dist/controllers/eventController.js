...
exports.eventController = {
    ...
    async create(req, res) {
        try {
            const { title, description, date, location, tags, maxAttendees } = req.body;
    ...
    async update(req, res) {
        try {
            ...
            const updatedEvent = await Event_1.Event.findByIdAndUpdate(req.params.id, {
                title,
                description,
                date,
                location,
    ...
    async search(req, res) {
        try {
            ...
            const events = await Event_1.Event.find(searchQuery)
                .populate("organizer", "name department avatar")
                .populate("attendees", "name department avatar")
                .sort({ date: 1 })
                .skip(skip)
                .limit(limit);
            const total = await Event_1.Event.countDocuments(searchQuery);
            res.json({
                events,
                currentPage: page,
                totalPages: Math.ceil(total / limit),
